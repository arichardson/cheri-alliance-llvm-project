...
static uint64_t addRelaSz(const RelocationBaseSection &relaDyn) {
  ...
  if (ctx.in.relaDyn->getParent() == relaDyn.getParent())
    size += ctx.in.relaDyn->getSize(ctx);
  return size;
}
...
DynamicSection<ELFT>::computeContents() {
  ...
  if (isMain && (ctx.in.relaPlt->isNeeded(ctx) || ctx.in.relaDyn->isNeeded(ctx))) {
  ...
  if (ctx.arg.emachine == EM_MIPS) {
    ...
    if (ctx.in.mipsCheriCapTable && ctx.in.mipsCheriCapTable->isNeeded(ctx)) {
    ...
    if (ctx.in.mipsCheriCapTableMapping &&
        ctx.in.mipsCheriCapTableMapping->isNeeded(ctx)) {
    ...
    if (ctx.in.capRelocs && ctx.in.capRelocs->isNeeded(ctx)) {
...
void RelocationBaseSection::finalizeContents(Ctx &) {
  ...
  if (ctx.in.relaPlt.get() == this && ctx.in.gotPlt->getParent()) {
    ...
    if (ctx.in.relaDyn.get() == this) {
      if (ctx.in.igotPlt && ctx.in.igotPlt->isNeeded(ctx)) {
...
template <class ELFT> void elf::createSyntheticSections(Ctx &ctx) {
  ...
  if (ctx.arg.capabilitySize > 0) {
    ctx.in.capRelocs = std::make_unique<CheriCapRelocsSection>("__cap_relocs");

    if (ctx.arg.emachine == EM_MIPS) {
      ctx.in.mipsCheriCapTable = std::make_unique<MipsCheriCapTableSection>();
