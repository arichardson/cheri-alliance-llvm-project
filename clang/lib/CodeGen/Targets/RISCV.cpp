...
RValue RISCVABIInfo::EmitVAArg(CodeGenFunction &CGF, Address VAListAddr,
  ...
  if (getTarget().hasFeature("cheri-bounded-vararg") &&
      getContext().getTargetInfo().areAllPointersCapabilities())
    return EmitVAArgCap(CGF, VAListAddr, Ty, Slot);
...
RValue RISCVABIInfo::EmitVAArgCap(CodeGenFunction &CGF, Address VAListAddr,
                                   QualType Ty, AggValueSlot Slot) const {
  int ArgGPRsLeft = 0, ArgFPRsLeft = 0;
  ABIArgInfo AI =
      classifyArgumentType(Ty, /*IsFixed=*/false, ArgGPRsLeft, ArgFPRsLeft);
  llvm::Type *BaseTy = CGF.ConvertType(Ty); // convert type for mem??
  ...
  if (AI.isIndirect()) {
    ...
    OnStackAddr = Address(CGF.Builder.CreateLoad(OnStackAddr, "vaarg.addr"),
