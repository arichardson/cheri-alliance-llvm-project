...
static bool hasFnAttributes(const CodeGenIntrinsic &Int) {
  return !Int.canThrow || Int.isNoReturn || Int.isNoCallback || Int.isNoSync ||
         ...
         Int.isStrictFP || Int.hasSideEffects || getEffectiveME(Int) != MemoryEffects::unknown();
}
...
static AttributeSet getIntrinsicFnAttributeSet(LLVMContext &C, unsigned ID) {
  ...
  case {}:
    ...
    if (Int.hasSideEffects)
      addAttribute("HasSideEffects");
    if (Int.isStrictFP)
...
void IntrinsicEmitter::EmitIntrinsicToBuiltinMap(
  ...
  for (const CodeGenIntrinsic &Int : Ints) {
    ...
    if (BuiltinName.empty() && Int.GCCBuiltinAliasName.empty())
      continue;
    ...
    if (!BuiltinName.empty()) {
      if (!Map.insert({BuiltinName, Int.EnumName}).second) {
        PrintFatalError(Int.TheDef->getLoc(),
                        "Intrinsic '" + Int.TheDef->getName() +
                            "': duplicate " + CompilerName + " builtin name!");
      }
    } else if (!Int.GCCBuiltinAliasName.empty()) {
      if (!Map.insert(std::make_pair(Int.GCCBuiltinAliasName,
                                     Int.EnumName)).second) {
        PrintFatalError("Intrinsic '" + Int.TheDef->getName() +
                            "': duplicate GCC builtin name!");
      }
      BuiltinName = Int.GCCBuiltinAliasName;
    } else
      assert(false && "Should either have a BuiltinName or GCCBuiltinAliasName.");
