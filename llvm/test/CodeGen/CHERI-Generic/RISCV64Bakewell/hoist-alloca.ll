...
; CHECK:       # %bb.0: # %entry
; CHECK-NEXT:    caddi csp, csp, -688
; CHECK-NEXT:    sc cra, 672(csp) # 16-byte Folded Spill
; CHECK-NEXT:    sc cs0, 656(csp) # 16-byte Folded Spill
; CHECK-NEXT:    sc cs1, 640(csp) # 16-byte Folded Spill
; CHECK-NEXT:    sc cs2, 624(csp) # 16-byte Folded Spill
; CHECK-NEXT:    sc cs3, 608(csp) # 16-byte Folded Spill
; CHECK-NEXT:    sc cs4, 592(csp) # 16-byte Folded Spill
; CHECK-NEXT:    mv s0, a0
; CHECK-NEXT:    li s3, 0
...
; CHECK-NEXT:    lc cra, 672(csp) # 16-byte Folded Reload
; CHECK-NEXT:    lc cs0, 656(csp) # 16-byte Folded Reload
; CHECK-NEXT:    lc cs1, 640(csp) # 16-byte Folded Reload
; CHECK-NEXT:    lc cs2, 624(csp) # 16-byte Folded Reload
; CHECK-NEXT:    lc cs3, 608(csp) # 16-byte Folded Reload
; CHECK-NEXT:    lc cs4, 592(csp) # 16-byte Folded Reload
; CHECK-NEXT:    caddi csp, csp, 688
; CHECK-NEXT:    ret
entry:
  %buf1 = alloca [123 x i32], align 4, addrspace(200)
