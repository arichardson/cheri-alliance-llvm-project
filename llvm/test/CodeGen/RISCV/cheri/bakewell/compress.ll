...
; RUN:   | llvm-objdump --no-print-imm-hex -d -M no-aliases --mattr=+zcheripurecap - | FileCheck %s --check-prefix=CHECK-NORVC
...
; RUN: %riscv64_bakewell_purecap_llc -filetype=obj < %t.fnattr \
; RUN:   | llvm-objdump --no-print-imm-hex -d --mattr=+c -M no-aliases --mattr=+zcheripurecap - | FileCheck %s --check-prefix=CHECK-NORVC
