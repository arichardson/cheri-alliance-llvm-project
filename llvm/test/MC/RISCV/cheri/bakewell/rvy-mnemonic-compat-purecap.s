# RUN: llvm-mc %s -triple=riscv32 -mattr=+a,+f,+d,+zcheripurecap,+zcherihybrid,+cap-mode -riscv-no-aliases -show-encoding \
# RUN:     | FileCheck --allow-unused-prefixes -check-prefixes=CHECK,CHECK-32 %s
# RUN: llvm-mc %s -triple=riscv64 -mattr=+a,+f,+d,+zcheripurecap,+zcherihybrid,+cap-mode -riscv-no-aliases -show-encoding \
