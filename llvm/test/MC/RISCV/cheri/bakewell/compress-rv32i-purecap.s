...
# RUN:   | llvm-objdump --no-print-imm-hex  --triple=riscv32 --mattr=+c,+zcheripurecap,+cap-mode -d - \
# RUN:   | FileCheck -check-prefixes=CHECK-BYTES,CHECK-BYTES32,CHECK-ALIAS,CHECK-ALIASOBJ32 %s
# RUN: llvm-mc -triple riscv32 -mattr=+c,+zcheripurecap,+cap-mode -filetype=obj < %s \
# RUN:   | llvm-objdump --no-print-imm-hex  --triple=riscv32 --mattr=+c,+zcheripurecap,+cap-mode -d -M no-aliases - \
...
# RUN:   | llvm-objdump --no-print-imm-hex  --triple=riscv64 --mattr=+c,+zcheripurecap,+cap-mode -d - \
# RUN:   | FileCheck -check-prefixes=CHECK-BYTES,CHECK-BYTES64,CHECK-ALIAS,CHECK-ALIASOBJ64 %s
# RUN: llvm-mc -triple riscv64 -mattr=+c,+zcheripurecap,+cap-mode -filetype=obj < %s \
# RUN:   | llvm-objdump --no-print-imm-hex  --triple=riscv64 --mattr=+c,+zcheripurecap,+cap-mode -d -M no-aliases - \
